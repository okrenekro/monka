...
\def\Plabel{Kräfte-\\GG\\$\sum F\!=\!0$}
\def\AOne{Trägheit\\(1.\,Newton)}
\def\ATwo{actio$=$\\reactio}
\def\AThree{Galilei-Inv.}
\def\AFour{$\omega_n$\\Eigenfreq.}
\def\AFive{$R\!\in\!SO(3)$}
\def\ASix{$\tfrac12 mv^2+\tfrac12 kx^2$}
\def\ANine{6 DOF}
\def\AFourteen{Modal-\\zerlegung}
\def\ATwelve{mech.\\Impedanz}
\def\AEight{$\sigma\!=\!\sigma^{\!\top}$\\Spannung}
\def\ATen{Modale\\Entartung}
\def\ASeven{$\nabla\!\cdot\!F$\\Kraftquelle}
\def\AEleven{$W\!=\!\!\int\!F\!\cdot\!ds$}
\def\AThirteen{Massen-\\dichte $\rho$}
\begin{center}\begin{tikzpicture}[scale=1.0]\wheel\end{tikzpicture}\end{center}

\newpage

%% =========================================================
%% 17. ELEKTRIK (SCHALTUNGSTHEORIE)
%% =========================================================
\section*{Elektrik (Schaltungstheorie)}
\def\Plabel{Arbeits-\\punkt}
\def\AOne{offener\\Kreis}
\def\ATwo{Serie/\\Parallel}
\def\AThree{Y-$\Delta$-\\Trafo}
\def\AFour{$\omega_0\!=\!\tfrac{1}{\sqrt{LC}}$}
\def\AFive{reziprok}
\def\ASix{$\tfrac12 CV^2\!+\!\tfrac12 LI^2$}
\def\ANine{\#\,Maschen}
\def\AFourteen{Netzwerk-\\Spektrum}
\def\ATwelve{$Z(s)$\\Impedanz}
\def\AEight{$Y\!=\!Y^{\!\top}$}
\def\ATen{LC-Resonanz}
\def\ASeven{$\sum i_k\!=\!0$\\Kirchhoff}
\def\AEleven{$P\!=\!\langle v,i\rangle$}
\def\AThirteen{Topologie-\\Rang}
\begin{center}\begin{tikzpicture}[scale=1.0]\wheel\end{tikzpicture}\end{center}

\newpage

%% =========================================================
%% 18. ELEKTROMAGNETISMUS (MAXWELL)
%% =========================================================
\section*{Elektromagnetismus (Maxwell)}
\def\Plabel{Vakuum-\\Lösung}
\def\AOne{$\nabla\!\cdot\!E\!=\!0$\\$\nabla\!\times\!B\!=\!0$}
\def\ATwo{$F^{\mu\nu}$\\Feldtensor}
\def\AThree{Lorentz-Inv.}
\def\AFour{Polari-\\sations-Mode}
\def\AFive{$A\!\to\!A{+}\partial\chi$\\Eichtrafo}
\def\ASix{$\tfrac12(\varepsilon_0 E^2{+}B^2/\mu_0)$}
\def\ANine{dim$\!=\!4$}
\def\AFourteen{TE/TM-\\Modenbasis}
\def\ATwelve{$(\Box)^{-1}$\\d'Alembert}
\def\AEight{$T^{\mu\nu}\!=\!T^{\nu\mu}$}
\def\ATen{Polar.-\\Entartung}
\def\ASeven{$\nabla\!\cdot\!E\!=\!\rho/\varepsilon_0$}
\def\AEleven{$\int\!A\!\cdot\!B\,dV$}
\def\AThirteen{Feld-\\stärke-Rg.}
\begin{center}\begin{tikzpicture}[scale=1.0]\wheel\end{tikzpicture}\end{center}
...
%% 19. OPTIK
%% =========================================================
\section*{Optik (Strahl- und Wellenoptik)}
\def\Plabel{Brennpkt /\\Bildebene}
\def\AOne{Schatten /\\Auslöschung}
\def\ATwo{$L_1\!\circ\!L_2$}
\def\AThree{$\det(ABCD)\!=\!1$}
\def\AFour{Resonator-\\Mode}
\def\AFive{Polarisations-\\Drehung}
\def\ASix{$I\!=\!|E|^2$}
\def\ANine{\#\,Moden}
\def\AFourteen{Modenbasis}
\def\ATwelve{Beugungs-\\Green $G$}
\def\AEight{Rezipro-\\zität}
\def\ATen{Laser-Linien-\\Entartung}
\def\ASeven{Strahlungs-\\quelle}
\def\AEleven{$\int\!\psi_1^{\!*}\psi_2\,dA$}
\def\AThirteen{Rayleigh-\\Auflösung}
\begin{center}\begin{tikzpicture}[scale=1.0]\wheel\end{tikzpicture}\end{center}
...
%% =========================================================
\section*{Akustik}
\def\Plabel{Stehende\\Welle}
\def\AOne{Schall-\\schatten}
\def\ATwo{$\sum p_k$\\Überlagerung}
\def\AThree{Energie-\\erhaltung}
\def\AFour{$f_n$\\Eigenfreq.}
\def\AFive{phasen-\\inv.\,Refl.}
\def\ASix{$\tfrac12\rho|v|^2$\\$+\tfrac{p^2}{2\rho c^2}$}
\def\ANine{\#\,Moden}
\def\AFourteen{Modenbasis}
\def\ATwelve{Helmholtz-\\Green}
\def\AEight{Rezipro-\\zität}
\def\ATen{Modale\\Entartung}
\def\ASeven{Monopol-\\Quelle}
\def\AEleven{$\langle p,v\rangle$\\Leistung}
\def\AThirteen{Modendichte\\$\rho(f)$}
\begin{center}\begin{tikzpicture}[scale=1.0]\wheel\end{tikzpicture}\end{center}
...
%% 21. HAPTIK
%% =========================================================
\section*{Haptik (Tastsinn)}
\def\Plabel{Sensor.\\Adaption}
\def\AOne{Reiz-\\schwelle}
\def\ATwo{multimodale\\Kombination}
\def\AThree{Weber-\\Konstanz}
\def\AFour{Vibrations-\\Kanal}
\def\AFive{Beidhand-\\Symmetrie}
\def\ASix{Reiz-\\Energiedichte}
\def\ANine{4 Rezeptor-\\klassen}
\def\AFourteen{Rezeptor-\\Spektrum}
\def\ATwelve{Haut$\to$\\Cortex-TF}
\def\AEight{sym.\\Tonotopie}
\def\ATen{Vibrations-\\Resonanz}
\def\ASeven{Druck-\\Quelle}
\def\AEleven{$\langle r_1,r_2\rangle$}
\def\AThirteen{Innervations-\\Dichte}
\begin{center}\begin{tikzpicture}[scale=1.0]\wheel\end{tikzpicture}\end{center}
